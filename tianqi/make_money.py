            ...
            if message_str.find('orderListResult') != -1 and message_str.find('TraderT') != -1:
                ...
                if (message_bytes_new != message_bytes_unique):
                    ...
                    if status == 'FILLED':
                        ...
                        print(symbol + ' ' + type + ' '+ side + ' ' + amount + ' ' + positionSide)
                        amount = float(amount)/50
                        try :
                            binance.create_order(symbol=symbol, type='market', side=side, amount=amount, positionSide=positionSide)
